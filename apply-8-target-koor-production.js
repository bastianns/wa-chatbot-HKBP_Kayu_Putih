 ...
 *
 * PROSEDUR KEAMANAN:
 * 1. Membuat file backup bertimestamp: `absensi.db.bak.<timestamp>`
 * 2. Memverifikasi integritas backup (bisa dibuka & dibaca) sebelum menyentuh produksi.
 * 3. Menjalankan seluruh koreksi di dalam SATU `db.transaction()` atomik.
 * 4. Menjalankan 5 assertion otomatis post-commit langsung pada `absensi.db`.
 * =========================================================================================
 */
...
const BACKUP_DB_PATH = path.resolve(`./absensi.db.bak.${timestamp}`);

console.log('=================================================================================');
console.log('📦 1. MEMBUAT DAN MEMVERIFIKASI BACKUP DATABASE PRODUKSI:');
console.log('=================================================================================');
...
// 1.a Copy file utama dan file WAL/SHM jika ada
fs.copyFileSync(PROD_DB_PATH, BACKUP_DB_PATH);
if (fs.existsSync(PROD_DB_PATH + '-wal')) {
  fs.copyFileSync(PROD_DB_PATH + '-wal', BACKUP_DB_PATH + '-wal');
}
if (fs.existsSync(PROD_DB_PATH + '-shm')) {
  fs.copyFileSync(PROD_DB_PATH + '-shm', BACKUP_DB_PATH + '-shm');
}

console.log(`✅ Backup file dibuat: ${BACKUP_DB_PATH}`);

// 1.b Verifikasi integritas file backup (Buka dan uji baca)
const backupDb = new Database(BACKUP_DB_PATH, { readonly: true });
const backupMembersCount = backupDb.prepare('SELECT COUNT(*) as cnt FROM members').get().cnt;
const backupAttendanceCount = backupDb.prepare('SELECT COUNT(*) as cnt FROM attendance_records').get().cnt;
const backupEventsCount = backupDb.prepare('SELECT COUNT(*) as cnt FROM events').get().cnt;
backupDb.close();

console.log(`🔍 Verifikasi Integritas Backup:`);
console.log(`   • Total Members    : ${backupMembersCount} baris`);
console.log(`   • Total Attendance : ${backupAttendanceCount} baris`);
console.log(`   • Total Events     : ${backupEventsCount} baris`);

if (backupMembersCount === 0 || backupAttendanceCount === 0) {
  console.error('❌ Error Kritis: File backup terbaca kosong atau rusak! Eksekusi dibatalkan.');
  process.exit(1);
}
console.log('✅ File backup TERVERIFIKASI VALID dan siap dipakai jika rollback diperlukan.\n');

// 2. Definisi 8 Anggota Target Koor
...
const db = new Database(PROD_DB_PATH);
db.pragma('journal_mode = WAL');
db.pragma('foreign_keys = ON');

const preState = {
  attendanceBefore: new Map(),
  deletedLosers: [],
...
console.log('=================================================================================');
console.log('⏳ 2. MENJALANKAN TRANSAKSI ATOMIK (db.transaction) PADA DATABASE PRODUKSI:');
console.log('=================================================================================');
...
console.log('\n✅ Transaksi Atomik Sukses Dieksekusi pada absensi.db PRODUKSI!\n');

// ---------------------------------------------------------------------------------
// 3. ASSERTION OTOMATIS LANGSUNG PADA DATABASE PRODUKSI
// ---------------------------------------------------------------------------------
console.log('=================================================================================');
console.log('🧪 3. MENJALANKAN 5 PROGRAMMATIC ASSERTIONS PADA DATABASE PRODUKSI:');
console.log('=================================================================================');

function assert(condition, assertionName, detailMsg) {
  if (!condition) {
    console.error(`\n❌ [ASSERTION FAILED PADA PRODUKSI] ${assertionName}`);
...
for (const tm of TARGET_MEMBERS) {
  ...
  if (tm.oldSlotPhone && tm.oldSlotPhone !== tm.correctPhone && tm.oldSlotPhone !== 'NO_PRIOR_PHONE_SLOT' && tm.oldSlotPhone !== '6281210176004') {
...
console.log('\n🎉 SELURUH 5 PROGRAMMATIC ASSERTIONS PADA DATABASE PRODUKSI BERHASIL 100%!\n');

// ---------------------------------------------------------------------------------
// 4. OUTPUT TABEL BEFORE / AFTER UNTUK HUMAN REVIEW
// ---------------------------------------------------------------------------------
console.log('=================================================================================');
console.log('📊 4. TABEL BEFORE / AFTER PRODUKSI:');
console.log('=================================================================================');
...
console.log('\n=================================================================================');
console.log('📜 5. LOG AUDIT TRAIL 8 ROW BARU DI lid_mapping_history PRODUKSI:');
console.log('=================================================================================');
